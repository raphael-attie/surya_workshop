...
# Silence specifically the BLANK keyword warning from Astropy
...
# This class is modified from FlareDSDataset
class CHDSDataset(HelioNetCDFDataset):
    """
    ...
    Additional Args:
        ...
        ds_ch_index_path: Path to the downstream flare intensity CSV index.
    ...
    Raises:
        ValueError: If ``ds_ch_index_path`` is not provided, or if no overlap exists
    ...
    def __init__(
        ...
        ds_ch_index_path: str | None = None, # R: was ds_flare_index_path in template
        ds_ch_mask_base_path: str | None = None, # R: new, was not in template
        ...
        **kwargs,
        ...
        if ds_ch_index_path is not None:
            self.ds_index = pd.read_csv(ds_ch_index_path)
        else:
            raise ValueError("ds_ch_index_path must be provided for CHDSDataset")

        if ds_ch_mask_base_path is not None:
            self.ds_ch_mask_base_path = ds_ch_mask_base_path
        else:
            raise ValueError("ds_ch_mask_base_path must be provided for CHDSDataset")
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        """
        # R: standardization occurs in the getitem
        ...
        sample["mask"] = self._load_mask(idx) #None # Load the masks as numpy array
        return sample

    def _load_mask(self, idx: int) -> np.ndarray:
        """
        Load the mask file for the given dataset index.
        ...
        """
        # Get the mask file path from the ds_index DataFrame at the same row
        # that corresponds to this sample's ds_index timestamp
        mask_relative_path = self.df_valid_indices.iloc[idx]["mask"]
        
        # Construct the full path to the mask file
        mask_full_path = Path(self.ds_ch_mask_base_path, mask_relative_path)
        
        # Load the mask file (assuming FITS format based on your imports)
